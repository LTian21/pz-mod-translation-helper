    ...
    static class MinGitHelper
    {
        ...
        private static bool CheckSpam(string line, LogSpamState state)
        {
            ...
            if (line.Contains("mode change", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Console.WriteLine("[Git] ���ڽ��� mode change ����...");
            ...
            if (line.Contains("create mode", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Console.WriteLine("[Git] ���ڽ��� create mode ����...");
            ...
            if (line.Contains("delete mode", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Console.WriteLine("[Git] ���ڽ��� delete mode ����...");
        ...
        public static string GetGitExecutablePath()
        {
            ...
            if (!OperatingSystem.IsWindows())
            {
                ...
                if (!string.IsNullOrEmpty(systemGit))
                {
                    ...
                    _cachedGitInfo = $"使用系统 Git: {systemGit}";
                    return _cachedGitPath;
                }
            }

            string errorMessage = OperatingSystem.IsWindows()
                ? $"未找到内置 Git (MinGit)，请确认程序目录下存在 {mingitPath}。"
                : "未找到系统 Git，请先安装 git（例如：sudo apt install git 或 brew install git）。";
        ...
        private static void PrepareSandboxEnvironment(ProcessStartInfo startInfo, bool enableProxy, string? proxyUrl)
        {
            ...
            if (enableProxy && !string.IsNullOrWhiteSpace(proxyUrl))
            {
                ...
                startInfo.Environment["https_proxy"] = proxyUrl;
                startInfo.Environment["all_proxy"] = proxyUrl;
                Console.WriteLine($"[��Ϣ] Git ʹ�ô���: {proxyUrl}");
            }
            else
            {
        ...
        private static void CheckForNetworkError(string stderr)
        {
            ...
            {
                Console.WriteLine("[����] Git TLS ����ʧ�� (schannel �ѽ��ã�ʹ�� OpenSSL ���³���)");
                throw new GitNetworkException($"Git TLS ����ʧ��: {stderr}");
            }
        }
        ...
        public static async Task<bool> CloneAsync(string repoUrl, string targetPath, string pat, bool useProxy = true)
        {
            try
            {
                Console.WriteLine($"[��ʼ] ��¡�ֿ�: {repoUrl}");
                Console.WriteLine($"  Ŀ��·��: {targetPath}");
                ...
                {
                    Console.WriteLine("[�ɹ�] �ֿ��¡�ɹ�");
                    return true;
                }
                else
                {
                    CheckForNetworkError(result.StandardError);
                    Console.WriteLine($"[����] ��¡ʧ�� (�˳���: {result.ExitCode})");
                    Console.WriteLine($"  ������Ϣ: {result.StandardError}");
                    return false;
                }
            }
            ...
            {
                Console.WriteLine($"[����] ��¡�����з����쳣: {ex.Message}");
                return false;
            }
        }
        ...
        public static async Task<bool> FetchAsync(string repoPath, string pat, string remote = "origin", bool force = false, bool prune = false)
        {
            try
            {
                Console.WriteLine($"[��ʼ] ��ȡԶ�̸���: {remote}");
                ...
                if (result.ExitCode == 0)
                {
                    Console.WriteLine("[�ɹ�] Զ�̸��»�ȡ�ɹ�");
                    return true;
                }
                else
                {
                    CheckForNetworkError(result.StandardError);
                    Console.WriteLine($"[����] ��ȡʧ�� (�˳���: {result.ExitCode})");
                    return false;
                }
            }
            catch (GitNetworkException)
            ...
            {
                Console.WriteLine($"[����] ��ȡ�����з����쳣: {ex.Message}");
                return false;
            }
        }
        ...
        public static async Task<bool> PullAsync(string repoPath, string pat, string remote = "origin", string branch = null)
        {
            try
            {
                ...
                if (branch == null)
                {
                    ...
                    {
                        Console.WriteLine("[����] �޷���ȡ��ǰ��֧��Ϣ");
                        return false;
                    }
                    Console.WriteLine($"[��Ϣ] ��ǰ��֧: {branch}");
                }
                ...
                else
                {
                    ...
                    {
                        Console.WriteLine("[����] ��ȡʧ��: ���ֺϲ���ͻ");
                        Console.WriteLine("[��ʾ] ����ϵ������Ա������ͻ");
                        return false;
                    }
                    else
                    {
        ...
        public static async Task<bool> StageAllAsync(string repoPath)
        {
            ...
            {
                Console.WriteLine($"[����] �ݴ�ʧ��: {ex.Message}");
                return false;
            }
        }
            ...
            string userEmail)
        {
            try
            {
                ...
                if (commitResult.ExitCode == 0)
                {
                    ...
                    Console.WriteLine($"[�ɹ�] �ύ�ɹ�: {commitSha}");
                    return (true, commitSha);
                }
                else
                {
                    Console.WriteLine($"[����] �ύʧ�� (�˳���: {commitResult.ExitCode})");
                    return (false, null);
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[����] �ύ�����з����쳣: {ex.Message}");
                return (false, null);
            }
        }
        ...
        public static async Task<bool> PushAsync(string repoPath, string pat, string remote, string branch, bool force)
        {
            try
            {
                ...
                {
                    Console.WriteLine("[����] ����ʧ��: Զ�˷�֧���µ��ύ");
                    Console.WriteLine("[��ʾ] ��ִ�� sync ����ͬ��������");
                    Console.WriteLine("[��ʾ] ������г�ͻ����ϵ������Ա");
                    return false;
                }

                Console.WriteLine($"[����] ����ʧ�� (�˳���: {result.ExitCode})");
                Console.WriteLine("[��ʾ] �����������ӻ��Ժ�����");
                return false;
            }
            catch (Exception ex)
            {
            ...
            bool fetchFirst = true)
        {
            try
            {
                ...
                if (result.ExitCode == 0)
                {
                    Console.WriteLine($"[�ɹ�] �Ѵ���/���Ǳ��ط�֧: {branch} (���� {remote}/{branch})");
                    return true;
                }

                Console.WriteLine($"[����] �������ط�֧ʧ�� (�˳���: {result.ExitCode})");
                return false;
            }
            catch (GitNetworkException)
            {
            ...
            {
                Console.WriteLine($"[����] ȷ�����ط�֧״̬ʧ��: {ex.Message}");
                return false;
            }
        }
        ...
        public static async Task<bool> ResetToRemoteAsync(string repoPath, string remote = "origin", string branch = "main")
        {
            try
            {
                Console.WriteLine($"[����] ǿ��ͬ����Զ�˷�֧: {remote}/{branch}");
                Console.WriteLine("[����] �⽫�������б����޸ģ�");

                var result = await RunGit($"reset --hard {remote}/{branch}", false, null, repoPath);

                if (result.ExitCode == 0)
                {
                    Console.WriteLine("[�ɹ�] ��ǿ��ͬ����Զ�˷�֧");
                    return true;
                }
                return false;
            }
        ...
        public static async Task<bool> RemoteSetUrlAsync(string repoPath, string remoteName, string newUrl)
        {
            try
            {
                Console.WriteLine($"[��ʼ] ����Զ�� '{remoteName}' �� URL Ϊ: {newUrl}");
                var args = $"remote set-url {remoteName} \"{newUrl}\"";
                var result = await RunGit(args, false, null, repoPath);

                if (result.ExitCode == 0)
                {
                    Console.WriteLine("[�ɹ�] Զ�� URL ���óɹ���");
                    return true;
                }
                else
                {
                    Console.WriteLine($"[����] ����Զ�� URL ʧ�� (�˳���: {result.ExitCode})");
                    Console.WriteLine($"  ������Ϣ: {result.StandardError}");
                    return false;
                }
            }
            ...
            {
                Console.WriteLine($"[����] ����Զ�� URL ʱ�����쳣: {ex.Message}");
                return false;
            }
        }
        ...
        public static async Task<bool> CheckoutAsync(string repoPath, string branchName, bool createIfNotExists = false)
        {
            try
            {
                Console.WriteLine($"[��ʼ] �л���֧: {branchName}");
                ...
                if (result.ExitCode == 0)
                {
                    Console.WriteLine($"[�ɹ�] ���л�����֧: {branchName}");
                    return true;
                }

                Console.WriteLine($"[����] �л���֧ʧ�� (�˳���: {result.ExitCode})");
                return false;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[����] �л���֧�����з����쳣: {ex.Message}");
                return false;
            }
        }
