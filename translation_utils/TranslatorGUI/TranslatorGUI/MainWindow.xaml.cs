...
namespace 翻译工具
{
    public partial class MainWindow : System.Windows.Window
    {
        ...
        // SSL/证书相关错误提示标记，避免重复弹窗
        private bool _warnedUnknownCertLookup;
        ...
        private void ShowUserDialog()
        {
            ...
            contentStack.Children.Add(chkUseMirror);
            ...
            btnOk.Click += async (s, e) =>
            {
                ...
                {
                    MessageBox.Show(dlg, "用户名只能包含字母、数字和下划线，且不能为空。", "无效用户名", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                {
                    MessageBox.Show(dlg, "请输入有效的邮箱地址。", "无效邮箱", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (string.IsNullOrEmpty(path) || !Directory.Exists(path))
                {
                    MessageBox.Show(dlg, "请选择有效的文件夹路径。", "无效路径", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                _config.UseMirrorSiteFirstDownload = chkUseMirror.IsChecked == true; // 保存勾选状态
                SaveConfig();
                ...
                // 初始化时，只在首次下载需要镜像时传入 UseMirrorSite
                string? initCommit = _config.UseMirrorSiteFirstDownload ? "UseMirrorSite" : null;
                await RunHelperAsync("init", initCommit);
        ...
        private void AppendOutput(string line)
        {
            ...
            // 同步检测常见 SSL/证书相关异常，并以弹窗形式提示用户
            TryNotifySslOrCertIssues(line);
        }
        ...
        private void ClearOutput()
        {
            try
            {
                ...
                // 重置提示标记，允许在新一轮操作中再次提示
                _warnedUnknownCertLookup = false;
                _warnedRevocationUnverified = false;
                _warnedSslConnectionFailed = false;
            }
            catch { }
        }
        ...
        private void TryNotifySslOrCertIssues(string line)
        {
            var l = line?.ToLowerInvariant();
            if (string.IsNullOrEmpty(l)) return;

            if (!_warnedUnknownCertLookup && l.Contains("unknown certificate lookup failure"))
            {
                _warnedUnknownCertLookup = true;
            ...
            if (!_warnedRevocationUnverified && l.Contains("certificate revocation status could not be verified"))
            {
                _warnedRevocationUnverified = true;
                Dispatcher.BeginInvoke(new Action(() =>
                {
                    MessageBox.Show(this,
            ...
            if (!_warnedSslConnectionFailed && l.Contains("the ssl connection could not be established"))
            {
                _warnedSslConnectionFailed = true;
                Dispatcher.BeginInvoke(new Action(() =>
                {
                    MessageBox.Show(this,
                        "无法建立安全连接，通常是由于网络问题，如数据丢失或网站被屏蔽。建议您切换网络环境、重试连接，或者使用代理软件。",
                        "网络连接提示",
                        MessageBoxButton.OK,
